  ...
  describe('All strings should be translated', function() {
    ...
    for (let i=1;i<4;i++) {
      ...
      it(`homepage.row1col${i}text`, function() {
        cy.get(`[data-test=row1-${i}] p > span`).should('contain', i18n[`homepage.row1col${i}text`].slice(0, 10))
      })
    }
    ...
    for (let box of ['patterns', 'showcase', 'blog']) {
      ...
      it(`intro.txt-${box}`, function() {
        if (box !== 'patterns') cy.get(`[data-test=${box}] p`).should('contain', i18n[`intro.txt-${box}`])
      })
