...
describe('User rights', function() {
  ...
  it('Consent (translation only)', function() {
    ...
    for (let string of strings) cy.get(`[data-test=${string}]`).should('contain', i18n[`gdpr.${string}`])
    let htmlStrings = [
      'compliant',
      'profileWhatAnswer',
      'profileWhyAnswer',
    ...
    for (let string of strings) cy.get(`[data-test=${string}]`).should('contain', i18n[`gdpr.${string}`].slice(0, 3))
    cy.get('[data-test=openDataQuestion]').click({force: true})
  ...
  it('Restrict (translation only)', function() {
    ...
    cy.get('blockquote.note').should('contain', i18n['account.restrictProcessingOfYourDataInfo'].slice(0,10))
