...
const buildNavigation = (mdxPages, titles) => {
  ...
  for (let pattern of patternList) {
    tree['/patterns/'].children['/patterns/' + pattern + '/'] = {
      title: utils.translate(`patterns.${pattern}.title`)
    }
  }
  ...
  for (let pattern in patterns) {
    tree['/showcase/'].children['/showcase/patterns/'].children[
      '/showcase/patterns/' + pattern + '/'
    ] = { title: utils.capitalize(pattern) }
  }
