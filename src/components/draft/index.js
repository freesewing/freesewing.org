...
const DraftIndex = props => {
  ...
  if (
    ...
    if (chunks.length < 4) {
      if (chunks[1] === 'recreate')
        main = <SelectRecipeModelPage app={props.app} recipe={chunks[2]} slug={props.slug} />
      ...
      if (chunks.length === 4 && chunks[1] === 'recreate' && chunks[3] === 'replica') {
        ...
        if (chunks[1] === 'recreate')
          main = <DraftPage {...draftProps} recreate recipe={chunks[2]} model={chunks[4]} />
