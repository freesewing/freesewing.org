...
const color = (val, theme) => {
  ...
  if (val < 4 && val > -4) return (theme === 'dark') ? '#fab005' : '#82c91e'
...
const MeasurementGauge = ({val=0, size=24, theme='light'}) => {
  ...
  return (
    ...
    >
      <path
        style={{stroke: 'currentColor', fill: 'none', strokeWidth: 10, strokeLinecap: 'round', opacity: disabled ? 0.5 : 1}}
      ...
      />
      <path
        style={{stroke: 'currentColor', fill: 'none', strokeWidth: 5, strokeLinecap: 'round', opacity: disabled ? 0 : 1}}
        d="M 50 5 l 0 15"
      />
      { !disabled && (
        <path
          style={{fill: color(val, theme), strokeWidth: 5, stroke: theme === 'dark' ? '#212529' : '#f8f9fa'}}
