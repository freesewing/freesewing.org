...
const ModelIndex = props => {
  useEffect(() => {
    if (props.slug === "/models") {
      props.app.frontend.setTitle(<FormattedMessage id="app.models"/>);
    } else {
      ...
      if (chunks.length === 3) {
        ...
        props.app.frontend.setTitle(<FormattedMessage id={"app." + chunks[3]}/>);
        props.app.frontend.setCrumbs([modelsCrumb, {slug: "/models/"+model, title: props.app.models[model].name}]);
      } else if (chunks.length === 5) {
        props.app.frontend.setTitle(<FormattedMessage id={"measurements."+chunks[4]} />);
        props.app.frontend.setCrumbs([modelsCrumb, {slug: "/models/"+model, title: props.app.models[model].name}]);
      }
    }
  }, [props.slug]);
  ...
  if (props.slug === "/models") main = <ModelList app={props.app} />
    ...
    } else if (chunks.length === 5) main = <EditMeasurementPage model={model} measurement={chunks[4]} app={props.app} />
  }
