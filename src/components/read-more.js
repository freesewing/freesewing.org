...
const ReadMore = ({ root, recurse=false, link=false, box=false }) => {
  ...
  const renderDocsLevel = (slug, level) => {
    ...
    return <li><Link to={slug}>{titles[slug]}{url}</Link>{links}</li>
  }
  ...
  if (box) return <Blockquote type='note'><h5><FormattedMessage id='app.furtherReading' /></h5>{renderDocs(root)}</Blockquote>
