...
const Size = (props) => {
  const convertSize = (breasts, size) =>
    breasts ? sizeConversion.withBreasts[size] : sizeConversion.withoutBreasts[size]
    ...
    <div className={`size ${props.breasts ? 'breasts' : 'no-breasts'}`}>
      <div className="top">
        <h5>
          ...
          <span className="size">{convertSize(props.breasts, props.size)}</span>
        </h5>
      </div>
